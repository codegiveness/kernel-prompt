...
A single-skill repository: **kernel-prompt** — refine or compose a prompt into one paragraph that lands on first try.

## Structure

- `skills/engineering/kernel-prompt/` — the skill itself:
...
- The npm package name is `@codegiveness/kernel-prompt`. The package name `kernel-prompt` is not taken on npm — we claim it for this skill.
...
- `kernel-prompt update` pulls the latest version via npm. It's a thin npm wrapper, not a self-contained updater.
